...
urlValue = "https://www.smava.de/brokerage/2+Sicheres-Einloggen-mit-Zugangsdaten.html"
BROWSER.get(urlValue)


try:
    BROWSER.find_element_by_xpath('//h1[text() = "Bitte loggen Sie sich ein"]')
    print("FUNCTIONAL: Page looks good!")
except:
    print("FUNCTIONAL: Page sucks!")

# Setup custom config for PERF
CONFIG_FILE = os.path.join(os.path.abspath(os.path.dirname(__file__)),'', 'config_smavaZugangsdaten.yaml')
